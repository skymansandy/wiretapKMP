...
class FindMatchingRuleUseCaseTest {
    ...
    private val useCase = FindMatchingRuleUseCase(repo)

    @Test
    fun `returns null when repository yields no enabled rules`() = runTest {
    ...
    @Test
    fun `returns the first rule whose method and criteria both match`() = runTest {
    ...
    @Test
    fun `skips rules whose method does not match the request`() = runTest {
    ...
    @Test
    fun `wildcard method rule matches any request method`() = runTest {
    ...
    @Test
    fun `rule with no criteria never matches`() = runTest {
    ...
    @Test
    fun `header matchers are evaluated against the request headers`() = runTest {
        val needsAuth = rule(
            id = 1,
            method = "*",
