...
class SocketLogManagerImplTest {

    private val repository = mock<SocketRepository>(MockMode.autoUnit)
    private val logger = mock<WiretapLogger>(MockMode.autoUnit)
    private val hook = mock<WiretapNotificationHook>(MockMode.autoUnit)
    private val manager = SocketLogManagerImpl(repository, logger, hook)

    // ---- read flows --------------------------------------------------------

    @Test
    fun `flowAllSockets delegates to repository`() = runTest {
    ...
    @Test
    fun `flowSocketById delegates to repository`() = runTest {
    ...
    @Test
    fun `flowSocketMessagesById delegates to repository`() = runTest {
    ...
    @Test
    fun `flowPagedSocketsForSearchQuery delegates query through to repository`() = runTest {
    ...
    @Test
    fun `createSocket logs, copies the assigned id into the entry, and notifies hook`() = runTest {
    ...
    @Test
    fun `updateSocket forwards through repository, logger, hook`() = runTest {
    ...
    // ---- logSocketMsg: active cache + reopen flow --------------------------

    @Test
    fun `logSocketMsg without active cache inserts the message and notifies after re-fetch`() =
        runTest {
    ...
    @Test
    fun `logSocketMsg with active cache re-opens connection when entry is missing`() = runTest {
    ...
    @Test
    fun `logSocketMsg with no cache and missing entry still inserts the message but skips hook`() =
        runTest {
    ...
    // ---- updateSocket cache eviction --------------------------------------

    @Test
    fun `updateSocket with Closed status evicts the cached entry`() = runTest {
        // Seed cache.
        everySuspend { repository.logNew(any()) } returns 9L
