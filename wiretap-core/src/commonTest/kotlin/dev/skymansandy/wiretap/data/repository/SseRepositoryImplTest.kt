...
class SseRepositoryImplTest {
    ...
    private val dao = mock<SseLogsDao>(MockMode.autoUnit)
    private val repo = SseRepositoryImpl(dao)
    ...
    @Test
    fun `flowEventsForId maps every event entity`() = runTest {
    ...
    @Test
    fun `markReopened upserts via insertSseLogWithId`() = runTest {
        repo.markReopened(SseConnection(url = "x", timestamp = 0))
        verifySuspend { dao.insertSseLogWithId(any()) }
    }

    @Test
    fun `update passes populated fields to the dao update query`() = runTest {
    ...
    @Test
    fun `logEvent inserts the event and increments the count`() = runTest {
        val event = SseEvent(
            connectionId = 9,
            eventType = "message",
            data = "{}",
    ...
    @Test
    fun `clearAll deletes events and logs`() = runTest {
        repo.clearAll()
    ...
    @Test
    fun `clearClosed deletes closed events and logs`() = runTest {
        repo.clearClosed()
