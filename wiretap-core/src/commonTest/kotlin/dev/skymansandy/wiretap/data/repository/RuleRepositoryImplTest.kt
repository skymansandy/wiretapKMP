...
class RuleRepositoryImplTest {
    ...
    private val dao = mock<RulesDao>(MockMode.autoUnit)
    private val repo = RuleRepositoryImpl(dao)

    @Test
    fun `flowAll maps entities from the dao to domain rules`() = runTest {
    ...
    @Test
    fun `flowForQuery delegates the query string to the dao`() = runTest {
    ...
    @Test
    fun `flowById maps entity and propagates null`() = runTest {
    ...
    @Test
    fun `getById delegates to dao and maps null pass-through`() = runTest {
    ...
    @Test
    fun `getEnabledRules maps every entity returned by the dao`() = runTest {
    ...
    @Test
    fun `addRule delegates the serialized entity to the dao`() = runTest {
        repo.addRule(rule(id = 0))
        verifySuspend { dao.insert(any()) }
    }

    @Test
    fun `setEnabled true serializes to 1`() = runTest {
        repo.setEnabled(id = 5, enabled = true)
        verifySuspend { dao.updateEnabled(enabled = 1L, id = 5) }
    }

    @Test
    fun `setEnabled false serializes to 0`() = runTest {
    ...
    @Test
    fun `deleteAll delegates`() = runTest {
        repo.deleteAll()
        verifySuspend { dao.deleteAll() }
    }

    @Test
    fun `updateRule passes Mock action fields through and nulls throttle columns`() = runTest {
        val mockRule = rule(
            id = 7,
    ...
    @Test
    fun `updateRule passes Throttle action fields and nulls mock columns`() = runTest {
    ...
    @Test
    fun `updateRule passes MockAndThrottle fields`() = runTest {
        val combo = rule(
            id = 9,
    ...
    @Test
    fun `updateRule serializes empty header list to null`() = runTest {
