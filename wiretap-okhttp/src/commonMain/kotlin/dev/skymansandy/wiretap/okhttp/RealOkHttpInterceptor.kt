...
/**
 * Real implementation of [WiretapOkHttpInterceptorDelegate] that logs
 * HTTP requests/responses and evaluates mock/throttle rules.
 */
internal class RealOkHttpInterceptor : WiretapOkHttpInterceptorDelegate, KoinComponent {
    ...
    private suspend fun initSessionIfNeeded(config: WiretapConfig) {
    ...
    override fun intercept(chain: Interceptor.Chain, config: WiretapConfig): Response = runBlocking {
        if (!config.enabled) {
